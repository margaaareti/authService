...
import (
	...
	"Test_derictory/auth/storage/redisStorage"
...
func NewApp() *App {
	...
	//picturestorage
	picStorage := repository.NewPictureStorage(viper.GetString("pictureStorage.url"))

	userRepo := postgres.NewAuthPostgres(db)
	tokenStg := redisStorage.NewAuthRedis(rdb)
	homeRepo := homerepo.NewStudentRepo(db)
	fileStorage := homerepo.NewPictureStorage(picStorage, "www", viper.GetString("pictureStorage.url"))

	return &App{
		authUC:  service.NewAuthUseCase(userRepo, tokenStg),
		homeUC:  mainservice.NewHomeUseCase(homeRepo, fileStorage),
		redisDB: rdb,
	}
